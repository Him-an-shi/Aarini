...
export const CycleTrackerScreen = () => {
  ...
  const updateState = useCallback(
    async (nextCycles, serverPrediction) => {
  ...
  const loadCycles = useCallback(async () => {
    ...
    try {
      ...
      setPrediction(
        result.prediction || predictCycleLocally(result.cycles, user?.cycleLength || 28),
      )
  ...
  useEffect(() => {
    loadCycles()
    AsyncStorage.getItem('predictionNotificationsEnabled').then((value) =>
  ...
  useEffect(() => {
    if (!loading && cycles.length > 0) {
      detectAnomalies({ cycles, prediction, moodEntries: [], medications: [], symptoms: [] }).then(
        setHealthAlerts,
      )
    }
  ...
  const saveCycle = async () => {
    ...
    if (end < start)
      return Alert.alert('Check the dates', 'The end date must be on or after the start date.')
    if ((end - start) / 86400000 > 13)
      return Alert.alert('Check the dates', 'A period entry can be at most 14 days.')
    if (start > new Date())
      return Alert.alert('Check the dates', 'A period cannot begin in the future.')
  ...
  const dayStyle = (date) => {
    ...
    if (cycles.some((cycle) => dateInRange(key, cycle.startDate, cycle.endDate)))
      return styles.loggedDay
    if (dateInRange(key, prediction?.nextPeriodStart, prediction?.nextPeriodEnd))
      return styles.predictedDay
    if (dateInRange(key, prediction?.ovulationWindowStart, prediction?.ovulationWindowEnd))
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.metrics}>
              <View style={styles.metricCard}>
                ...
                <Text style={styles.muted}>
                  {prediction.averageCycleLength}-day average · {prediction.confidence} confidence
                </Text>
              </View>
              <View style={styles.metricCard}>
        ...
        <View style={styles.sectionHeader}>
          ...
          >
            ...
            <Text style={styles.smallButtonText}>Log</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.calendarCard}>
          ...
          <View style={styles.weekRow}>
            {t('cycleTracker.weekdays').map((day, index) => (
            ...
            {cycles.slice(0, 6).map((c) => (
              ...
              >
                <View style={styles.cycleRowInfo}>
                  <Text style={styles.cycleRowDates}>
        ...
        <View style={styles.notificationCard}>
          <View style={styles.bell} importantForAccessibility="no">
            <Bell size={19} color={colors.primaryDark} />
          </View>
          <View style={styles.flex}>
            <Text style={styles.notificationTitle}>Prediction reminders</Text>
            <Text style={styles.muted}>
              A heads-up before your predicted period and ovulation window.
            </Text>
          </View>
          <Switch
        ...
        <Text style={styles.disclaimer}>
          Estimates can vary and should not be used as birth control or medical diagnosis.
        </Text>
      </ScrollView>
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity
              ...
              disabled={saving}
              accessibilityRole="button"
              accessibilityLabel={saving ? 'Saving period' : 'Save period'}
            >
