...
export async function gatherAllData(userId, apiFetch) {
  ...
  const localCycles = JSON.parse(
    (await AsyncStorage.getItem(`cycles:${userId || 'local'}`)) || '[]',
  )
  ...
  const medications = JSON.parse(
    (await AsyncStorage.getItem(`medications:${userId || 'local'}`)) || '[]',
  )
  results.medications = medications.length > 0 ? medications : null

  const chatHistory = JSON.parse(
    (await AsyncStorage.getItem(`@aarini_chat_history:${userId || 'default'}`)) || '[]',
  )
...
export function formatReadableReport(data) {
    ...
    cycles.forEach((c, i) => {
      if (c.startDate) {
        if (c.endDate) {
          ...
          lines.push(
            `  ${i + 1}. ${c.startDate} to ${c.endDate} (${dur} days)${c.flowIntensity ? ` - Flow: ${c.flowIntensity}` : ''}`,
          )
        } else {
          lines.push(
            `  ${i + 1}. ${c.startDate} to ongoing${c.flowIntensity ? ` - Flow: ${c.flowIntensity}` : ''}`,
          )
        }
      }
    })
  ...
  if (data.moods && data.moods.length > 0) {
    ...
    data.moods.forEach((m) => {
      lines.push(
        `  ${m.date || m.timestamp}: ${m.mood || m.rating || 'N/A'}${m.note ? ` - ${m.note}` : ''}`,
      )
    })
    lines.push('')
  }
