...
function interpretLag(lag, r) {
  ...
  if (lag > 0) return `${strength} ${sign} correlation: mood changes ${lag} day(s) before cycle phase shifts`;
...
export function computeDirectionalAnalysis(moodSeries, cycleSeries, lag = 2) {
  ...
  return {
    ...
    lag,
    interpretation: direction === 'mood_causes_cycle'
