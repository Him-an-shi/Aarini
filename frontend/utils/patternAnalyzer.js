...
export function analyzeCycleRegularity(cycles) {
  ...
  if (sorted.length < 2) {
    return {
      ...
      details: 'Log at least 2 cycles for regularity analysis.',
    }
  }
  ...
  if (intervals.length < 2) {
    return {
      ...
      details: 'More consistent cycle logging needed.',
    }
  }
...
export function analyzeSymptomPatterns(symptoms) {
  ...
  const severityTrend = sortedDates.map((date) => {
    ...
    const avgSeverity = mean(
      daySymptoms.map((s) => severityMap[s.severity?.toLowerCase()] || 0).filter(Boolean),
    )
...
export function calculateCycleQualityScore(cycles, symptoms, moods) {
  ...
  const rawScore =
    (regularity.score / 5) * regularityWeight +
    dataCompleteness * completenessWeight +
    trackingConsistency
  ...
  if (finalScore >= 80) interpretation = 'Excellent tracking habits with regular patterns.'
    ...
    interpretation = 'Good awareness. Continue consistent logging for better insights.'
  else if (finalScore >= 40)
    interpretation = 'Building awareness. Try logging symptoms and mood daily.'
    ...
    for (let i = sortedCycles.length - 1; i >= 0; i--) {
      ...
      const nextStart =
        i < sortedCycles.length - 1
          ...
          : new Date(cycleStart.getTime() + avgCycle * DAY_MS)
